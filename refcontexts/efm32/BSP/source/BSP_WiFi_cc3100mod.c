...
static SPI_DMA_Callback_T WiFi_CC3100MOD_SPI_TxDmaCallback = NULL;
static SPI_DMA_Callback_T WiFi_CC3100MOD_SPI_RxDmaCallback = NULL;
...
Retcode_T BSP_WiFi_CC3100MOD_Connect(void)
{
    ...
    USART0_PORT_BASE->ROUTE = USART_ROUTE_RXPEN | USART_ROUTE_TXPEN | USART_ROUTE_CLKPEN | USART_ROUTE_LOCATION_LOC0;
    ...
    BSP_WiFi_CC3100MOD_SPI_Handle.Instance = USART0_PORT_BASE;
    return RETCODE_OK;
}
...
Retcode_T BSP_WiFi_CC3100MOD_Enable(BSP_WiFi_CC3100MOD_InterruptCallback_T callback)
{
    ...
    if (NULL == BSP_WiFi_CC3100MOD_SPI_Handle.IrqRxCallback || NULL == BSP_WiFi_CC3100MOD_SPI_Handle.IrqTxCallback || NULL == callback)
    {
        return RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_BSP_WIFI_SERIAL_NOT_INITIALIZED);
    }
    BoardCommon_WakeupPowerSupply2V5(WIFI_CC3100MOD);

    NVIC_DisableIRQ(GPIO_EVEN_IRQn);

    WiFi_CC3100MOD_ISRCallback = callback;

    BoardCommon_GPIOInterruptRegisterCallback(WIFI_INT_PIN, WiFi_INT_InterruptCallback);
    ...
    BoardCommon_Delay(WIFI_SUPPLY_SETTING_DELAY_MS);

    GPIO_PinOutClear(WIFI_NRESET_PORT, WIFI_NRESET_PIN);
    BoardCommon_Delay(WIFI_MIN_RESET_DELAY_MS);
    GPIO_PinOutSet(WIFI_NRESET_PORT, WIFI_NRESET_PIN);
    BoardCommon_Delay(WIFI_PWRON_HW_WAKEUP_DELAY_MS);
    OStoBSPdelayFunc(WIFI_INIT_DELAY_MS);

    /*enable the clock for the USART interface*/
    CMU_ClockEnable(WIFI_SPI0_CLOCK, true);
    USART0_PORT_BASE->CMD = USART_CMD_CLEARTX | USART_CMD_CLEARRX;
    ...
    WiFi_CC3100MOD_SPI_RxISRCallback = BSP_WiFi_CC3100MOD_SPI_Handle.IrqRxCallback;
    WiFi_CC3100MOD_SPI_TxISRCallback = BSP_WiFi_CC3100MOD_SPI_Handle.IrqTxCallback;

#if BCDS_FEATURE_DMA

    WiFi_CC3100MOD_SPI_RxDmaCallback = BSP_WiFi_CC3100MOD_SPI_Handle.DmaRxCallback;
    WiFi_CC3100MOD_SPI_TxDmaCallback = BSP_WiFi_CC3100MOD_SPI_Handle.DmaTxCallback;
#endif
...
Retcode_T BSP_WiFi_CC3100MOD_Disable(void)
{
    ...
    BoardCommon_GPIOInterruptUnregisterCallback(WIFI_INT_PIN);
    WiFi_CC3100MOD_SPI_RxISRCallback = NULL;
    WiFi_CC3100MOD_SPI_TxISRCallback = NULL;
    ...
    BoardCommon_SnoozePowerSupply2V5(WIFI_CC3100MOD);
    BoardCommon_Delay(WIFI_POWER_OFF_DELAY_MS);
...
Retcode_T BSP_WiFi_CC3100MOD_Wakeup(void)
{
    ...
    GPIO_PinOutSet(WIFI_NHIB_PORT, WIFI_NHIB_PIN);
    BoardCommon_Delay(WIFI_HIB_WAKEUP_DELAY_MS);
    return RETCODE_OK;
}
...
Retcode_T BSP_WiFi_CC3100MOD_Hibernate(void)
{
    ...
    GPIO_PinOutClear(WIFI_NHIB_PORT, WIFI_NHIB_PIN);
    BoardCommon_Delay(WIFI_MIN_HIB_DELAY_MS);
    return RETCODE_OK;
}
...
{
    if (NULL != WiFi_CC3100MOD_SPI_RxISRCallback)
    {
        /* All UART interrupt processing is to be done at the MCU level */
        WiFi_CC3100MOD_SPI_RxISRCallback((SPI_T) &BSP_WiFi_CC3100MOD_SPI_Handle);
    }
}
...
{
    if (NULL != WiFi_CC3100MOD_SPI_TxISRCallback)
    {
        /* All UART interrupt processing is to be done at the MCU level */
        WiFi_CC3100MOD_SPI_TxISRCallback((SPI_T) &BSP_WiFi_CC3100MOD_SPI_Handle);
    }
...
void WiFi_CC3100MOD_SPI_RxDMAComplete(uint32_t channel, bool primary, void * user)
{
    ...
    if (NULL != WiFi_CC3100MOD_SPI_RxDmaCallback)
    {
        /* All UART interrupt processing is to be done at the MCU level */
        WiFi_CC3100MOD_SPI_RxDmaCallback((SPI_T) &BSP_WiFi_CC3100MOD_SPI_Handle);
    }
}
...
void WiFi_CC3100MOD_SPI_TxDMAComplete(uint32_t channel, bool primary, void * user)
{
    ...
    if (NULL != WiFi_CC3100MOD_SPI_TxDmaCallback)
    {
        /* All UART interrupt processing is to be done at the MCU level */
        WiFi_CC3100MOD_SPI_TxDmaCallback((SPI_T) &BSP_WiFi_CC3100MOD_SPI_Handle);
    }
}
