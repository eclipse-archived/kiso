    ...
    switch (buttonId)
    {
        ...
        else
        {
            ...
            if (1 == BSP_XDK_KEY1_INTERRUPT)
            {
                ...
                /*lint -e506 wrong config results in runtime error reported*/
                else if ((1 != BSP_XDK_KEY1_INTERRUPT_FORCE) && (NULL != BoardCommon_GPIOInterruptGetCallback(KEY1_PIN)))
                {
                ...
                else
                {
                    ...
                    BoardCommon_GPIOInterruptRegisterCallback(KEY1_PIN, Button1InterruptCallback);
        ...
        else
        {
            ...
            if (1 == BSP_XDK_KEY2_INTERRUPT)
            {
                ...
                /*lint -e506 wrong config results in runtime error reported*/
                else if ((1 != BSP_XDK_KEY2_INTERRUPT_FORCE) && (NULL != BoardCommon_GPIOInterruptGetCallback(KEY2_PIN)))
                {
                    retcode = RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_BSP_BUTTON_INTERRUPT_BUSY);
                }
                else
                {
                    ...
                    BoardCommon_GPIOInterruptRegisterCallback(KEY2_PIN, Button2InterruptCallback);
...
Retcode_T BSP_Button_Disable(uint32_t buttonId)
{
    ...
    switch (buttonId)
    {
        ...
        {
            BoardCommon_GPIOInterruptUnregisterCallback(KEY1_PIN);
            XDK_ButtonState.Button1UsesInterrupt = false;
        }
        break;
        case BSP_XDK_BUTTON_2:
        ...
        {
            BoardCommon_GPIOInterruptUnregisterCallback(KEY2_PIN);
            XDK_ButtonState.Button2UsesInterrupt = false;
        }
        break;
        default:
