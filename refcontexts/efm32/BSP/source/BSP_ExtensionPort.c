...
static bool CheckIrqNotRegistred(BSP_ExtensionPort_PinId_T pinID)
{
    ...
    callback = BoardCommon_GPIOInterruptGetCallback(BSP_ExtensionPortTable[pinID].Pin);
    if (NULL != callback)
    {
        return false;
    }
...
Retcode_T BSP_ExtensionPort_RegisterGpioInt(BSP_ExtensionPort_PinId_T pinID, BSP_ExtensionPort_GpioIntMode_T interruptMode, BSP_ExtensionPort_GpioCallback_T gpioAppCallBack)
{
    ...
    {
        BoardCommon_GPIOInterruptRegisterCallback((uint8_t) BSP_ExtensionPortTable[pinID].Pin, (GPIOINT_IrqCallbackPtr_t) gpioAppCallBack);
...
Retcode_T BSP_ExtensionPort_DisableGpio(BSP_ExtensionPort_PinId_T pinID)
{
    ...
    if (RETCODE_OK == retVal)
    {
        ...
        if ( true == BSP_ExtensionPortInstance[pinID].gpioEnableInterrupt)
        {
            ...
            BoardCommon_GPIOInterruptUnregisterCallback(BSP_ExtensionPortTable[pinID].Pin);
...
Retcode_T BSP_ExtensionPort_Disconnect(void)
{
    ...
    BoardCommon_DisablePowerSupply3V3(EXTENSION_BOARD);
    TristateAllExtensionPortGpios();
    return retVal;
}
...
Retcode_T BSP_ExtensionPort_EnableSpi(void)
{
    ...
    if((Bsp_ExtensionPortSpiHandle->IrqRxCallback != NULL) && (Bsp_ExtensionPortSpiHandle->IrqTxCallback != NULL))
    {
        Bsp_ExtensionPortSpiRxIsrCb = Bsp_ExtensionPortSpiHandle->IrqRxCallback;
        Bsp_ExtensionPortSpiTxIsrCb = Bsp_ExtensionPortSpiHandle->IrqTxCallback;
#if BCDS_FEATURE_DMA
        Bsp_ExtensionPortSpiRxDmaCb = Bsp_ExtensionPortSpiHandle->DmaRxCallback;
        Bsp_ExtensionPortSpiTxDmaCb = Bsp_ExtensionPortSpiHandle->DmaTxCallback;
#endif
    ...
    {
        retVal = RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_BSP_EXTENSIONPORT_SPI_MCU_IRQ_CALLBACK_NULL);
    }
    return retVal;
}
...
Retcode_T BSP_ExtensionPort_DisableSpi(void)
{
    ...
    Bsp_ExtensionPortSpiRxIsrCb = NULL;
    Bsp_ExtensionPortSpiTxIsrCb = NULL;
#if BCDS_FEATURE_DMA
    Bsp_ExtensionPortSpiRxDmaCb = NULL;
    Bsp_ExtensionPortSpiTxDmaCb = NULL;
#endif
    return retVal;
}
...
{
    Retcode_T retVal = RETCODE_OK;
    if ((NULL != Bsp_ExtensionPortSpiRxIsrCb) && (NULL != Bsp_ExtensionPortSpiHandle))
    {
        /* All SPI interrupt processing is to be done at the MCU level */
        Bsp_ExtensionPortSpiRxIsrCb((SPI_T) Bsp_ExtensionPortSpiHandle);
    }
    else
    {
        retVal = RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_NULL_POINTER);
...
{
    Retcode_T retVal = RETCODE_OK;
    if ((NULL != Bsp_ExtensionPortSpiTxIsrCb) && (NULL != Bsp_ExtensionPortSpiHandle))
    {
        /* All SPI interrupt processing is to be done at the MCU level */
        Bsp_ExtensionPortSpiTxIsrCb((SPI_T) Bsp_ExtensionPortSpiHandle);
    }
    else
    {
        retVal = RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_NULL_POINTER);
