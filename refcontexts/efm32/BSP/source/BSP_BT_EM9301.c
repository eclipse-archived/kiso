...
Retcode_T BSP_BT_EM9301_Enable(void)
{
    ...
    BoardCommon_WakeupPowerSupply2V5(BT_EM9301);
    if (RETCODE_OK == retcode)
    {
        /*
         * Wait for the BT module to start up
         */
        BoardCommon_Delay(BT_STARTUP_DELAY_MS);
        /*
...
Retcode_T BSP_BT_EM9301_SetWUHigh(void)
{
    ...
    if (BSP_UNLOCKED == BT_GPIOLock)
    {
        ...
        BoardCommon_Delay(BT_WAKEUP_DELAY_MS);
    }
    else
    {
...
{
    if (NULL != BSP_BT_EM9301_UART_Handle.IrqTxCallback)
    {
...
{
    if (NULL != BSP_BT_EM9301_UART_Handle.IrqRxCallback)
    {
        /* All UART interrupt processing is to be done at the MCU level */
