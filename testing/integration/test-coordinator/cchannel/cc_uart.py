...
class CCUart(cchannel_interface.CChannel):
...
def test_communication_Send_check():
    ...
    message_to_send = message.Message(msg_type=message.MessageType.ACK, sub_type=message.MessageAckType.ACK, test_section=1, test_suite=2, test_case=3)
    # Initialize and open uart
    ch = CCUart(serialPort='COM3', baudrate=9600)
    ch.cc_open()
    # Send message
...
def test_communication_Receive_check():
    """ Test to do with HW connected or UART emulator """
    message_sent = message.Message(msg_type=message.MessageType.COMMAND, sub_type=message.MessageCommandType.TEST_CASE_SETUP, test_section=1, test_suite=2, test_case=3)
    # Initialize and open uart
    ch = CCUart(serialPort='COM3', baudrate=9600)
    ch.cc_open()
    # Wait for a message to be sent: it is C0 78 5E 60 01 00 00 01 02 03 00 C0
    ack_message_received = ch._cc_receive(timeout=30)
...
if __name__ == '__main__':
    # Test 1
    test_communication_Send_check()
    # Test 2
    test_communication_Receive_check()
