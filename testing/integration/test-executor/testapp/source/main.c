...
/**
 * @brief This file implements the #main() of the test executor software. After
 * initializing the test board and starting the OS scheduler, it shall call the
 * function in the feature integration tests responsible for initializing and
 * starting the tests (e.g. #TestEntry_Initialize() under
 * core/essentials/test/integration/source/TestEntry.c)
 *
 * @file
 */
 ...
 * @details This function will execute before the scheduler starts and it is
 * intended to make the target board ready for operation.
 ...
 * @details This function is called when Retcode_RaiseError() function is
 * invoked, it is used to report the error to the user.
 * @param[in] error Error code raised.
 * @param[in] isfromIsr if true then the ErrorHandler is being executed from an
 * ISR context and not all the services are available.
 */
 ...
 * @details This function is called when ever the Systick IRQ is hit. This is a
 * temporary implementation where the #SysTick_Handler() is not directly mapped
 * to #xPortSysTickHandler(). Instead it is only called if the scheduler has
 * started.
 */
static void SysTickPreCallback(void)
{
...
/**
 * @brief If static allocation is supported then the application must provide
 * the following callback function
 * @details Enables the application to optionally provide the memory that will
 * be used by the idle task as the task's stack and TCB.
 */
void vApplicationGetIdleTaskMemory(StaticTask_t **ppxIdleTaskTCBBuffer,
                                   StackType_t **ppxIdleTaskStackBuffer,
                                   uint32_t *pulIdleTaskStackSize)
...
/**
 * @brief If static allocation and timers are supported then the application
 * must provide the following callback function
 * @details Enables the application to optionally provide the memory that will
 * be used by the timer task as the task's stack and TCB.
 */
void vApplicationGetTimerTaskMemory(StaticTask_t **ppxTimerTaskTCBBuffer,
                                    StackType_t **ppxTimerTaskStackBuffer,
                                    uint32_t *pulTimerTaskStackSize)
