...
static void dispatcher(CCMsg_T *ccmsg, uint32_t unusedParameter)
{
    ...
    {
        //@todo dei9bue: handle the fact that the type is not supported
...
static void processAck(CCMsg_T *ccmsg)
{
    ...
    {
        //@todo dei9bue: do we just ignore it? or raise an error?
    }
...
static void processCommand(CCMsg_T *ccmsg)
{
    ...
    if (testRunner.waitingForAck)
    {
         ...
         * so we can jsut stop the timer and continue
         */
    ...
    case TEST_ENTRY_SETUP:
        ...
        else
        {
             ...
             * This has a Jira issue: COSP-562. */
