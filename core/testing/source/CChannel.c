 ...
 *
 * @brief
 * This module is responsible for the transmission and reception of data through the communication medium, which can be 
 * UART / UDP / BLE and so on.
 */
/*###################### INCLUDED HEADERS ###########################################################################*/
...
Retcode_T CChannel_Initialize(void)
{
    ...
    {
        char msg[18] = "CCHANNEL INIT OK\r\n";
        retcode = Serial_Send((void *)msg, 18);
    }
    return retcode;
}
...
{
    if (CCMSG_VERSION != ((buffer[0] & CCMSG_VERSION_MASK) >> 6))
    {
