...
{
    /**
     * @brief Modem handle associated with this socket.
     */
    uint32_t Id;

    /**
     * @brief Marks if this socket context locally allocated.
     */
    bool IsCreatedLocally;

    /**
     * @brief Marks if this socket context has a valid modem representation.
     */
    bool IsCreatedOnModem;
    ...
    /**
     * @brief Data-Ready callback associated with this socket.
     */
    CellularSocket_NotifyDataReady_T OnDataReady;

    /**
     * @brief Connection-Accepted callback associated with this socket.
     */
    CellularSocket_NotifyConnectionAccepted_T OnConnectionAccepted;

    /**
     * @brief Protocol associated with this socket.
     */
    CellularSocket_Protocol_T Protocol;

    /**
     * @brief Local port associated with this socket. If set to zero a random port was
     * selected.
     */
    uint16_t LocalPort;
};
...
/**
 * @brief Create and bind a new socket.
 *
 * This function is preparing an internal socket context which is bound to a
 * given address and can later be used in the @e listen() or @e connect() API.
 *
 * To be encapsulated by via #Engine_Dispatch().
 *
 * @param[in,out] param
 * A valid pointer to a #CellularSocket_CreateAndBindParam_S structure that
 * contains the details needed to create and bind the socket. On completion the
 * @e CreatedHandle field will contain the newly created socket handle.
 *
 * @param len
 * Length of #CellularSocket_CreateAndBindParam_S structure.
 *
 ...
 *
 * This function will translate the internal socket context and allocate the
 * socket resources on the modem-internal TCP/IP stack.
 *
 * To be encapsulated by via #Engine_Dispatch().
 *
 * @param[in] param
 * A valid pointer to a #CellularSocket_ConnectParam_S structure that contains
 * details needed to connect the socket to a remote host.
 *
 * @param[in] len
 * Length of #CellularSocket_ConnectParam_S structure.
 *
 * @return A #Retcode_T indicating the result of the procedure.
 */
 ...
 * To be encapsulated by via #Engine_Dispatch().
 *
 * @note This also works with UDP sockets provided the socket got 'connected'
 * before.
 *
 * @param[in] param
 * A valid pointer to a #CellularSocket_SendToParam_S structure that contains
 * details needed to send data over the given socket.  RemoteIp and RemotePort
 * fields are ignored.
 *
 * @param[in] len
 * Length of #CellularSocket_SendToParam_S structure.
 *
 * @return A #Retcode_T indicating the result of the procedure.
 */
 ...
 *
 * @param[in,out] param
 * A valid pointer to a #CellularSocket_ReceiveFromParam_S structure. Data will
 * be received from the declared socket and stored stored inside the provide
 * data array.
 *
 * @param[in] len
 * Length of #CellularSocket_ReceiveFromParam_S structure.
 *
 * @return A #Retcode_T indicating the result of the procedure.
 */
 ...
 * automatically be accepted and forwarded onto a new socket published via the
 ...
 *
 * @param[in] param
 * A valid pointer to a #CellularSocket_ListenParam_S structure.
 *
 * @param[in] len
 * Length of #CellularSocket_ListenParam_S structure.
 *
 * @return A #Retcode_T indicating the result of the procedure.
 */
 ...
 *
 * @param[in,out] param
 * A valid pointer to a #CellularSocket_Context_S structure that contains the
 * details of the to-be-closed socket. After successful completion the IsValid
 * field will be unset.
 *
 * @param len
 * Length of #CellularSocket_Context_S structure.
 *
 * @return A #Retcode_T indicating the result of the procedure.
 */
 ...
 *
 * @param[out] freeSlot
 * Will be set to the index of the free socket context inside the #Sockets pool.
 *
 * @return A #Retcode_T indicating the result of the procedure.
 */
 ...
 *
 * @param[in] socket
 * Handle of the driver socket context.
 *
...
static Retcode_T CreateAndBind(void *param, uint32_t len)
{
    ...
    if (RETCODE_OK == retcode)
    {
        retcode = At_Set_USOCR(&usocr, &ctx->Id);
    }
...
static Retcode_T Connect(void *param, uint32_t len)
{
    ...
    retcode = CelToUbloxAddr(connectParam->RemoteIp, &usoco.RemoteAddr);

    if (RETCODE_OK == retcode)
    {
        retcode = At_Set_USOCO(&usoco);
    }
...
static Retcode_T SendTo(void *param, uint32_t len)
{
    ...
    Retcode_T retcode = CelToUbloxAddr(sendToParam->RemoteIp, &usostParam.RemoteIp);

    if (RETCODE_OK == retcode)
    {
        retcode = At_Set_USOST(&usostParam, &usostResp);
    }
    return retcode;
}
...
static Retcode_T Receive(void *param, uint32_t len)
{
    ...
    Retcode_T retcode = At_Set_USORD(&usordParam, &usordResp);
    if (RETCODE_OK == retcode)
    {
...
static Retcode_T ReceiveFrom(void *param, uint32_t len)
{
    ...
    Retcode_T retcode = At_Set_USORF(&usorfParam, &usorfResp);
    if (RETCODE_OK == retcode)
    {
        *recvParam->BytesReceivedOrAvailable = usorfResp.Length;
        if (0 < usorfParam.Length)
        {
            *recvParam->RemotePort = usorfResp.RemotePort;
            retcode = UbloxToCelAddr(&usorfResp.RemoteIp, recvParam->RemoteIp);
        }
        else
        {
                                        ...
                                        uint16_t listeningPort)
{
    ...
    if (RETCODE_OK == retcode)
    {
        ...
        retcode = UbloxToCelAddr(remoteIp, &celRemoteIp);
    }
    ...
    CellularSocket_NotifyDataReady_T onDataReady)
{
    ...
    else
    {
        ...
        return Engine_Dispatch(CreateAndBind, CELLULAR_SOCKET_SHORT_ENQUEUE_TIMEOUT, &param, sizeof(param));
    }
}
    ...
    uint16_t remotePort)
{
    ...
    else
    {
        ...
        param.RemoteIp = remoteIp;
        param.RemotePort = remotePort;
        return Engine_Dispatch(SendTo, CELLULAR_SOCKET_SHORT_ENQUEUE_TIMEOUT, &param, sizeof(param));
    }
}
    ...
    uint32_t *bytesReceived)
{
    ...
    else
    {
        ...
        return Engine_Dispatch(Receive, CELLULAR_SOCKET_SHORT_ENQUEUE_TIMEOUT, &param, sizeof(param));
    }
}
    ...
    uint16_t *remotePort)
{
    ...
    else
    {
        ...
        param.RemoteIp = remoteIp;
        param.RemotePort = remotePort;
        return Engine_Dispatch(ReceiveFrom, CELLULAR_SOCKET_SHORT_ENQUEUE_TIMEOUT, &param, sizeof(param));
    }
}
