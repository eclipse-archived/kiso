...
Retcode_T Cellular_QueryModem(Cellular_QueryInfo_T info, void* value)
{
    ...
    case CELLULAR_QUERYINFO_NETWORKINFO:
        ...
        retcode = CellularNetwork_QueryCellEnvironment(&data);

        if (RETCODE_OK == retcode)
        {
            Cellular_NetworkInfo_T *p = (Cellular_NetworkInfo_T *) value;
            memcpy(p->CountryCode, &data.Mcc, CELLULAR_COUNTRY_CODE_LENGTH);
            memcpy(p->NetworkCode, &data.Mnc, CELLULAR_NETWORK_CODE_LENGTH);
...
{
    return CellularSocket_Open(socket, CELLULAR_SOCKET_TYPE_UDP, callback);
}
...
{
    return Cellular_ResolveDomainName(DomainName, DomainNameLength, IpAddress);
}
