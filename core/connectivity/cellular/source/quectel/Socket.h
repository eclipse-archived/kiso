...
#ifndef QCTEL_SOCKET_H
#define QCTEL_SOCKET_H

#include "BCDS_Basics.h"
#include "BCDS_Retcode.h"
#include "BCDS_CellularUdpSocket.h"

#if UINT8_C(12) < CELLULAR_SOCKET_COUNT
#error "Quectel only supports 12 sockets maximum"
#endif
...
typedef enum
{
    CELLULAR_SOCKET_TYPE_TCP = 0,
    CELLULAR_SOCKET_TYPE_UDP,
    CELLULAR_SOCKET_TYPE_TCP_LISTENER,
    CELLULAR_SOCKET_TYPE_UDP_SERVICE,

    CELLULAR_SOCKET_TYPE_COUNT
} CellularSocket_Type_T;

/**
 * @brief The types of sockets QIURC
 */
typedef enum
{
    CELLULAR_SOCKET_QIURC_CLOSED = 0,
    CELLULAR_SOCKET_QIURC_RECEIVING,
    CELLULAR_SOCKET_QIURC_INCOMING,
    CELLULAR_SOCKET_QIURC_PDP_DEACTIVATED,

    CELLULAR_SOCKET_QIURC_COUNT
} CellularSocket_URC_T;
 ...
 *
 * @param[out] request The open socket request containing a socket handle to the newly opened socket
 *
...
/**
 * @brief Invalidates all available sockets
 *
 */
void CellularSocket_InvalidateAll(void);

/**
 * @brief Implements URC handling for QIURC events (pdp and socket status).
 *        This function never blocks longer than required to handle the URC.
 *        If no QIURC URC is available, this function returns immediately.
 *
