        ...
        AT_CXREG_AcT_T* value)
{
    ...
    if (RETCODE_OK == ret)
    {
        if ((((int32_t) AT_CXREG_ACT_GSM <= number)
                && ((int32_t) AT_CXREG_ACT_EUTRA_NR >= number))
                || ((int32_t) AT_CXREG_ACT_INVALID == number))
        {
...
static Retcode_T ExtractCgpaddrAddress(const uint8_t* addressBuff, uint32_t addressBuffLen, AT_CGPADDR_Address_T* parsedAddress)
{
    if (NULL == addressBuff || NULL == parsedAddress || 0 == addressBuffLen
            || addressBuffLen > AT_3GPP_27007_MAX_IP_STR_LENGTH)
    {
...
Retcode_T At_Test_AT(void)
{
    ...
    while (AtResponseQueue_GetEvent(CMD_3GPP_27007_SHORT_TIMEOUT, &e) == RETCODE_OK)
    {
        if (AT_EVENT_TYPE_MISC == e->Type && e->BufferLength > 0
                && 0 == memcmp(CMD_3GPP_27007_SET_AT, e->Buffer, e->BufferLength))
        {
            retcode = RETCODE_OK;
        }
        AtResponseQueue_MarkBufferAsUnused();
