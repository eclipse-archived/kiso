 ...
 * @file
 *
 * @brief Implements the interface for the AT Response Parser.
 */
...
/**
 * @brief the response parser state
 */
#ifdef GTEST
...
/**
 * @brief appends to the internal buffer
 */
...
/**
 * @brief Rolls back previously appended content
 */
...
/**
 * @brief changes the internal parser state
 */
...
static int32_t AtrpStateCmdarg(uint8_t* buffer, uint32_t len)
{
     ...
     * In case of the latter, a AT_DEFAULT_S3_CHARACTER will be part of the string
     * which is then trimmed by AtrpTrimWhitespace.
     */
    uint32_t status = 0;
 ...
 * Our implementation is according to spec, where OK has a higher priority than misc content, yet this is a source of problems/errors.
 */
...
/**
 * @todo: if the buffer overflows within one MISC message, we go to the error state.
 * Instead, we should trigger the MISC event and reset the buffer.
 */
static int32_t AtrpStateMiscContent(uint8_t* buffer, uint32_t len)
{
    ...
    if (status & (ATRP_CONSUME_STATUS_FOUND_DELIMITERA))
    {
        uint32_t NewLength;
        uint8_t* NewBuffer = AtrpTrimWhitespace(state.Buffer, state.BufferPosition, &NewLength);
        if (NewLength > 0 && NULL != ATRP_MISC_EVENT)
            ATRP_MISC_EVENT(NewBuffer, NewLength);
...
/**
 * @brief Resets the response parser to its initialized state. Call this from an error
 * 		  callback to restore normal parser operation.
 */
...
/**
 * @brief Registers the response code event callback which is called when a response code string is parsed.
 * 		  This generally marks the end of an AT command response - meaning that the AT command is finished
 * 		  upon reception of the response code.
 */
...
/**
 * @brief Registers the ERROR event callback which is called when a parsing error occurs.
 */
...
/**
 * @brief Registers the CMD event callback which is called when a command response is parsed.
 */
...
/**
 * @brief Registers the CMD ARG event callback which is called when a command argument is parsed.
 * 		  Any call to this callback will be preceded by an invocation of the CMD event handler.
 */
...
/**
 * @brief Registers the MISC event callback which is called when miscellaneous content is parsed.
 */
