...
struct MCU_SPI_Handle_S
{
    ...
    enum BCDS_HAL_TransferMode_E TransferMode;  /**<    Identifies the transfer
                                                        ...
                                                        SPI interface.*/
    SPI_IRQ_Callback_T IrqRxCallback;           /**<    Function pointer of the MCU
                                                        SPI IRQ RX callback handler which
                                                        is used by the BSP to notify
                                                        MCU about IRQ events. Will
                                                        be set by MCU SPI upon
                                                        initialization. */
    SPI_IRQ_Callback_T IrqTxCallback;           /**<    Function pointer of the MCU
                                                        SPI IRQ TX callback handler which
                                                        is used by the BSP to notify
                                                        MCU about IRQ events. Will
                                                        be set by MCU SPI upon
                                                        initialization. */
    SPI_DMA_Callback_T DmaRxCallback;          /**<    Function invoked by BSP in case IRQ and Rx DMA mode */
    SPI_DMA_Callback_T DmaTxCallback;          /**<    Function invoked by BSP in case IRQ and Tx DMA mode  */
