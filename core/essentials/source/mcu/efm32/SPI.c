...
static void MCU_SPI_IRQ_RX_Callback(SPI_T spi);
static void MCU_SPI_IRQ_TX_Callback(SPI_T spi);
static void MCU_SPI_DMA_RX_Callback(SPI_T spi);
static void MCU_SPI_DMA_TX_Callback(SPI_T spi);
...
static void MCU_SPI_IRQ_RX_CallbackDefault(SPI_T spi);
static void MCU_SPI_IRQ_TX_CallbackDefault(SPI_T spi);
static void MCU_SPI_DMA_RX_CallbackDefault(SPI_T spi);
static void MCU_SPI_DMA_TX_CallbackDefault(SPI_T spi);
 ...
 */
void MCU_SPI_IRQ_RX_CallbackDefault(SPI_T spi)
{
    BCDS_UNUSED(spi);
    /* if this happens hang on here */
    Retcode_RaiseError(RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_UNEXPECTED_BEHAVIOR));
}
/*
 * Calling this function should result in an error because the application
 * developer should have done sometihng wrong e.g called deinitializing SPI
 * without disabling the intended peripheral
 */
void MCU_SPI_IRQ_TX_CallbackDefault(SPI_T spi)
{
    BCDS_UNUSED(spi);
    /* if this happens hang on here */
    Retcode_RaiseError(RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_UNEXPECTED_BEHAVIOR));
 ...
 * without disabling the intended peripheral
 */
void MCU_SPI_DMA_RX_CallbackDefault(SPI_T spi)
{
    BCDS_UNUSED(spi);
    /* if this happens hang on here */
    Retcode_RaiseError(RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_UNEXPECTED_BEHAVIOR));
 ...
 * without disabling the intended peripheral
 */
void MCU_SPI_DMA_TX_CallbackDefault(SPI_T spi)
{
    BCDS_UNUSED(spi);
    /* if this happens hang on here */
    Retcode_RaiseError(RETCODE(RETCODE_SEVERITY_ERROR, RETCODE_UNEXPECTED_BEHAVIOR));
...
Retcode_T MCU_SPI_Initialize(SPI_T spi, MCU_SPI_Callback_T callback)
{
    ...
    if (BCDS_HAL_TRANSFER_MODE_INTERRUPT == spi_ptr->TransferMode)
    {
        ...
        spi_ptr->IrqTxCallback = MCU_SPI_IRQ_TX_Callback; /* invoked by BSP if spi in interrupt mode, SPI TX callback of MCU */
        spi_ptr->IrqRxCallback = MCU_SPI_IRQ_RX_Callback; /* invoked by BSP if spi in interrupt mode, SPI RX callback of MCU */

        spi_ptr->DmaTxCallback = MCU_SPI_DMA_TX_Callback;
        spi_ptr->DmaRxCallback = MCU_SPI_DMA_RX_Callback;
...
Retcode_T MCU_SPI_Deinitialize(SPI_T spi)
{
    ...
    spi_ptr->IrqTxCallback = MCU_SPI_IRQ_TX_CallbackDefault; /* invoked by BSP if spi in interrupt mode, SPI TX callback of MCU */
    spi_ptr->IrqRxCallback = MCU_SPI_IRQ_RX_CallbackDefault; /* invoked by BSP if spi in interrupt mode, SPI RX callback of MCU */
    spi_ptr->DmaRxCallback = MCU_SPI_DMA_RX_CallbackDefault; /* invoked by BSP if spi in DMA mode, callback of MCU */
    spi_ptr->DmaTxCallback = MCU_SPI_DMA_TX_CallbackDefault; /* invoked by BSP if spi in DMA mode, callback of MCU */
