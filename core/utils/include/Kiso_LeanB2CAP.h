 ...
 *
 * @details
 *      The LeanB2CAP is a binary framing protocol to transmit payload data
...
#ifndef KISO_LEANB2CAP_H_
#define KISO_LEANB2CAP_H_
...
/**
 * @brief
 *      Enum to represent the LeanB2CAP events.
 */
...
/**
 * @brief
 *      Type definition to represent the LeanB2CAP event.
 */
typedef enum LeanB2CAP_Event_E LeanB2CAP_Event_T;

/**
 * @brief
 *      Structure to represent any common data frames.
 */
...
/**
 * @brief
 *      Type definition to represent any common data frames.
 */
...
/**
 * @brief
 *      Type definition to represent the LeanB2CAP command type.
 */
typedef enum LeanB2CAP_CommandType_E LeanB2CAP_CommandType_T;

/**
 * @brief
 *      Structure to represent the raw data for LeanB2CAP frame building.
 */
...
/**
 * @brief
 *      Type definition to represent the raw data for LeanB2CAP frame building.
 */
...
/**
 * @brief
 *      Structure to represent LeanB2CAP payload overhead (header and footer).
 */
...
/**
 * @brief
 *      Type definition to represent LeanB2CAP payload overhead (header and footer).
 */
typedef struct LeanB2CAP_Overhead_S LeanB2CAP_Overhead_T;

/**
 * @brief
 *      This is the data type for the LeanB2CAP event callback function.
 *
 * @details
 *      The callback is registered at the initialization. It is invoked by the
...
/**
 * @brief
 *      Typedef for the LeanB2CAP handle.
 *
 * @details
 *      The actual handle context information are hidden
...
/**
 * @brief
 *      Initializes an LeanB2CAP instance.
 *
 * @param[out]  Handle
 ...
 *      If no free handle is available
 * @retval  #RETCODE_NULL_POINTER
 *      If NULL has been passed instead of a valid Callback
 * 
 */
...
/**
 * @brief
 *      Handles the LeanB2CAP received data processing.
 *
 * @details
 *      The processed data results are intimated to the user in the
 *      same processing context via the registered callback.
 *
 * @not
 *      The handle must be initialized prior to this by
 ...
 * @retval	#RETCODE_OK
 *      If successful
 * @retval 	#RETCODE_LEANB2CAP_RX_PROCESSOR_FAIL
 *      Unknown/un-handled application logic was encountered
 * @retval 	#RETCODE_FAILURE
 *      If internal CRC calculation failed in the last byte processed
 * @retval  #RETCODE_LEANB2CAP_SD_ERROR
 *      If invalid start delimiter error was encountered in the last byte processed
 * @retval  #RETCODE_LEANB2CAP_LENGTH_ERROR
 *      If invalid length was encountered in the last byte processed
 * @retval  #RETCODE_LEANB2CAP_CMD_TYPE_ERROR
 *      If invalid command type was encountered in the last byte processed
 * @retval  #RETCODE_LEANB2CAP_CRC_ERROR
 *      If invalid CRC was encountered in the last byte processed
 * @retval  #RETCODE_LEANB2CAP_FRAME_ERROR
 *      Unknown/un-handled frame state was encountered
 * @retval  #RETCODE_LEANB2CAP_INVALID_CALLBACK
 *      Callback for Lean B2CAP is NULL
 * @retval  #RETCODE_INVALID_PARAM
 *      If Unknown Handle is passed instead of a valid Handle
 * @retval  #RETCODE_NULL_POINTER
 *      NULL has been passed instead of a valid Handle
 * 
 */
...
/**
 * @brief
 *      Builds the LeanB2CAP payload.
 *
 * @note
 *      This is completely independent of the handle.
 ...
 *      If successful
 * @retval 	#RETCODE_FAILURE
 *      If internal CRC calculation failed
 * @retval  #RETCODE_LEANB2CAP_CMD_TYPE_ERROR
 *      If invalid command type was provided
 * @retval  #RETCODE_LEANB2CAP_LENGTH_ERROR
 *      If invalid length was provided
 * @retval  #RETCODE_NULL_POINTER
 *      NULL has been passed instead of a valid input / output payload pointer
 * 
 */
...
/**
 * @brief
 *      Builds the LeanB2CAP payload overhead (header and footer).
 *
 * @note
 *      This is completely independent of the handle.
 *      Also, the input and output data buffers must be provided
 ...
 *      NULL has been passed instead of a valid input / output payload pointer
 * @retval  #RETCODE_INVALID_PARAM
 *      If invalid output buffer sizes or unsupported raw data length is provided
 * 
 */
...
/**
 * @brief
 *      Resets the LeanB2CAP received data process information.
 *
 * @note
 *      The handle must be initialized prior to this by
 *      making use of the LeanB2CAP_Initialize API.
...
/**
 * @brief
 *      Deinitializes an LeanB2CAP instance.
 *
 * @note
 *      The handle must be initialized prior to this by
 *      making use of the LeanB2CAP_Initialize API.
