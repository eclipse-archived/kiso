 ...
 *          before including the BCDS_Logging.h:
 *
 * @code{.c}
...
#ifndef BCDS_LOGGING_H
#define BCDS_LOGGING_H
...
uint8_t LOG_LEVEL_VAR(BCDS_PACKAGE_ID) = (uint8_t) LOG_LEVEL_PACKAGE;
#else
__attribute__(( weak )) uint8_t LOG_LEVEL_VAR(BCDS_PACKAGE_ID) = (uint8_t) LOG_LEVEL_PACKAGE_DEFAULT;
...
uint8_t LOG_LEVEL_VAR(BCDS_MODULE_ID) = (uint8_t) LOG_LEVEL_MODULE;
#else
__attribute__(( weak )) uint8_t LOG_LEVEL_VAR(BCDS_MODULE_ID) = (uint8_t) LOG_LEVEL_MODULE_DEFAULT;
...
#define LOG(level, ...) { \
    if (level <= LOG_LEVEL_GET_CURRENT()) { \
        (void) Logging_Log(level, BCDS_PACKAGE_ID, BCDS_MODULE_ID, __FILE__, __LINE__, __VA_ARGS__); \
    } \
}
 ...
 */
extern uint8_t LOG_LEVEL_VAR(BCDS_PACKAGE_ID);

/**
 * @brief External reference to module log level variable.
 */
extern uint8_t LOG_LEVEL_VAR(BCDS_MODULE_ID);
