 ...
 *
 * @details
 *      By default the package and module log levels are set to LOG_LEVEL_PACKAGE_DEFAULT
...
/**
 * @brief
 *      A macro to get current package log level.
 *
 * @retval
 *      A package log level.
 */
...
/**
 * @brief
 *      A macro to get current module log level.
 *
 * @retval
 *      A module log level.
 */
...
/**
 * @brief
 *      A macro to get log level for the current package module.
 *
 * @return
...
/**
 * @brief
 *      A macro to set package log level in runtime.
 *
 * @param [in] package
 *      A package id.
 * @param [in] level
...
/**
 * @brief
 *      A macro to set module log level in runtime.
 *
 * @param [in] module
 *      A module id.
 * @param [in] level
...
/**
 * @brief
 *      External reference to package log level variable.
 */
extern uint8_t LOG_LEVEL_VAR(KISO_PACKAGE_ID);

/**
 * @brief
 *      External reference to module log level variable.
 */
extern uint8_t LOG_LEVEL_VAR(KISO_MODULE_ID);

/**
 * @brief
 *      A function pointer type to log recorder initializer.
 */
typedef Retcode_T (*LogRecordInit_T)(void *self);

/**
 * @brief
 *      A function pointer type to log recorder deinitializer.
 */
typedef Retcode_T (*LogRecordDeinit_T)(void *self);

/**
 * @brief
 *      A function pointer type to log recorder writer.
 */
...
/**
 * @brief
 *      A function pointer type to log appender initializer.
 */
typedef Retcode_T (*LogAppenderInit_T)(void *wakeup);

/**
 * @brief
 *      A function pointer type to log appender writer.
 */
typedef Retcode_T (*LogAppenderWrite_T)(const char *message, uint32_t length);

/**
 * @brief
 *      A log recorder operations type.
 */
typedef struct
{
...
/**
 * @brief
 *      External reference to log asynchronous recorder.
 */
extern const LogRecorder_T *Logging_AsyncRecorder;

/**
 * @brief
 *      External reference to log synchronous recorder.
 */
extern const LogRecorder_T *Logging_SyncRecorder;

/**
 * @brief
 *      External reference to log UART appender.
 */
extern const LogAppender_T *Logging_UARTAppender;

/**
 * @brief
 *      External reference to log external flash appender.
 */
extern const LogAppender_T *Logging_ExtFlashMemAppender;

/**
 * @brief
 *      Initializes the log recorder and appender engines.
 *
 * @param [in] recorder
 *      A log recorder implementation.
 * @param [in] appender
 *      A log appender implementation.
 *
 * @retval RETCODE_OK
 *      If successfully initialized
 * @retval RETCODE_INVALID_PARAM
 *      If recorder pointer, any of recorder pointer attributes (see #LogRecorder_T)
 *      or appender pointer, any of appender pointer attributes (see #LogAppender_T) is null
 * @return
 *      Errors from recorder.init (see #LogRecordInit_T) or from appender.Init (see #LogAppenderInit_T)
 *      will be returned
 * 
 */
 ...
 *      If the logger has not been initialized previously
 * @retval #RETCODE_NOT_SUPPORTED
 *      If there is no matching filter (see #LogFilter_Apply)
 * @return #errors from recorder
 *      Write (see #LogRecorder_T.Write) will be returned
 * 
 */
...
/**
 * @brief
 *      Adds log filter item.
 *
 * @param [in] level
 *      A filter log level.
 * @param [in] package
 *      A filter log package id.
 * @param [in] module 
 *      A filter log module id.
 * @return
 ...
 * @retval #RETCODE_INVALID_PARAM
 *      If id is bigger than #LOG_FILTER_ITEM_COUNT
 * 
 */
...
/**
 * @brief
 *      Modifies existing log filter item.
 *
 * @param [in] id
 *      A filter id.
 * @param [in] level
 *      A new filter log level.
 * @param [in] package
 *      A new filter log package id.
 * @param [in] module
 *      A new filter log module id.
 *
 * @retval #RETCODE_OK
 *      If filter is successfully configured
 * @retval #RETCODE_INVALID_PARAM
 *      If id is bigger than #LOG_FILTER_ITEM_COUNT
 * 
 */
...
/**
 * @brief
 *      An internal function to check if there is matching filter present.
 *
 * @param [in] level
 *      A log level.
 * @param [in] package
 ...
 *
 * @return True
 *      If the matching filter present or the latter is inactive, false otherwise.
 * 
 */
